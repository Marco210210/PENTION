...
def plot_plan_view(C1, x, y, dispersion_placeholder, stability_class=1):
    with dispersion_placeholder:
        # porta a numpy
        if hasattr(C1, "detach"):
            C1 = C1.detach().cpu().numpy()
        else:
            C1 = np.asarray(C1)

        # 3D (m,m,t) -> integra nel tempo; 2D (m,m) -> usa direttamente
        if C1.ndim == 3:
            data = np.trapz(C1, axis=2) * 1e6  # μg/m³
            cbar_label = r'$\mu g \cdot m^{-3}$'
        elif C1.ndim == 2:
            data = C1
            cbar_label = 'arb. units'
        else:
            raise ValueError(f"C1 deve essere 2D o 3D, trovato {C1.ndim}D.")

        # limiti colore robusti
        vmin = np.nanpercentile(data, 5)
        vmax = np.nanpercentile(data, 95)

        fig, ax_main = plt.subplots(figsize=(8, 6))
        pcm = ax_main.pcolor(x, y, data, cmap='jet', shading='auto', vmin=vmin, vmax=vmax)
        fig.colorbar(pcm, ax=ax_main, label=cbar_label)
