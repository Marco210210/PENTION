...
def load_model(binary_map, m=500, device=None, pretrained_path=None):
    ...
    try:
        ...
        logger.warning(f"Model not available or failed to load ({e}). Fallback mode enabled.")
        return None  # <— Fallback: nessun modello
...
def correct_dispersion(wind_dir, wind_speed, concentration_map, building_map, global_feature=None, device=None, m=500, pretrained_path=None):
    ...
    # Aggrega nel tempo (m, m, T) -> (m, m)
    cm_agg = np.mean(concentration_map, axis=2).astype(np.float32)

    # Prova a caricare il modello
    model = load_model(building_map, m=m, device=device, pretrained_path=pretrained_path)

    # Fallback semplice se il modello non è disponibile
    if model is None:
        logger.info("Applying heuristic fallback correction (mask buildings, keep free cells).")
        # azzera sui building (0), conserva sulle celle libere (1)
        corrected = cm_agg * (building_map.astype(np.float32))
        return corrected

    # ====== percorso con modello ======
    ...
    with torch.no_grad():
        try:
            ...
            output = output.detach().cpu().numpy()[0]  # (m, m)
            logger.info(f"Inference completed. Output shape: {output.shape}")
        except Exception as e:
            logger.error(f"Error during model inference: {e}")
            # Se il modello crasha, ripieghiamo ancora sul fallback
            output = cm_agg * (building_map.astype(np.float32))
